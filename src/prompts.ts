...
async function processVersion(version: string, originalCwd: string) {
	...
	try {
		...
		try {
			...
			cmdOutput = execSync(`npx --node-options="--no-warnings" -y @mariozechner/claude-trace --claude-path ./package/cli.js --no-open --run-with -p "hey"`, {
				encoding: 'utf-8',
				stdio: ['pipe', 'pipe', 'pipe']
			});
		} catch (error: any) {
		...
		const nonHaikuRequest = data.find(pair => 
			pair.request?.body?.model && !pair.request.body.model.toLowerCase().includes("haiku")
		);
